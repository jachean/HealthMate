export default {
  // Navbar
  nav: {
    home: 'Home',
    doctors: 'Doctors',
    clinics: 'Clinics',
    login: 'Log in',
    createAccount: 'Create account',
    myProfile: 'My profile',
    logout: 'Log out',
  },

  // Home - Hero
  hero: {
    badge: 'Trusted by patients nationwide',
    title: 'Book medical appointments,',
    titleHighlight: 'stress-free',
    subtitle: 'Browse specialties and clinics without an account. Log in only when you\'re ready to confirm.',
    findDoctor: 'Find a doctor',
    createAccount: 'Create account',
    noBrowseAccount: 'No account needed to browse',
    freeToUse: 'Free to use',
    cardDoctor: 'Dr. Maria S.',
    cardSpecialty: 'Cardiology',
    cardConfirmed: 'Appointment Confirmed',
    cardTime: 'Tomorrow, 10:30 AM',
  },

  // Home - Trust Strip
  trust: {
    browseTitle: 'Browse without login',
    browseText: 'Search specialties and clinics freely. Log in only when confirming.',
    clinicsTitle: 'Multiple clinics',
    clinicsText: 'Browse options across clinics and book the time that fits.',
    fastTitle: 'Fast booking flow',
    fastText: 'Specialty, clinic, doctor, time slot, confirm. Five steps, done.',
  },

  // Home - Popular Specialties
  specialties: {
    title: 'Popular specialties',
    subtitle: 'A quick way to start your booking flow',
    browseDoctors: 'Browse doctors',
    cardiology: 'Cardiology',
    dermatology: 'Dermatology',
    pediatrics: 'Pediatrics',
    dentistry: 'Dentistry',
    neurology: 'Neurology',
    generalMedicine: 'General Medicine',
  },

  // Home - How It Works
  howItWorks: {
    title: 'How it works',
    subtitle: 'A simple booking flow designed for patients',
    step1Title: 'Pick a specialty',
    step1Text: 'Start broad \u2014 we\'ll narrow down clinics and doctors next.',
    step2Title: 'Choose clinic & doctor',
    step2Text: 'Compare options across associated clinics.',
    step3Title: 'Confirm time slot',
    step3Text: 'Log in only when confirming your appointment.',
  },

  // Home - CTA
  cta: {
    title: 'Ready to book your next appointment?',
    subtitle: 'Join thousands of patients who book their medical appointments through HealthMate. No fees, no hassle.',
    browseDoctors: 'Browse doctors',
    createFreeAccount: 'Create free account',
  },

  // Footer
  footer: {
    tagline: 'Medical appointment booking, simplified',
    description: 'Browse clinics and doctors, book appointments, and manage your visits in one place.',
    terms: 'Terms',
    privacy: 'Privacy',
    contact: 'Contact',
    bottomTagline: 'Clear, modern medical booking',
  },

  // Doctors Page
  clinics: {
    title: 'Our Clinics',
    subtitle: 'Browse our network of partner clinics',
    loadError: 'Could not load clinics. Please try again.',
    viewClinic: 'View clinic',
    noClinics: 'No clinics found.',
  },

  clinicProfile: {
    notFound: 'Clinic not found',
    notFoundHint: 'This clinic may no longer exist.',
    about: 'About this clinic',
    doctors: 'Doctors at this clinic',
    doctorCount: 'doctor | doctors',
    noDoctors: 'No doctors listed yet.',
    location: 'Location',
    book: 'Book',
  },

  doctors: {
    pageTitle: 'Find a Doctor',
    pageSubtitle: 'Browse our network of qualified healthcare professionals',
    searchPlaceholder: 'Search by name, clinic, city, or specialty...',
    filters: 'Filters',
    doctorFound: 'doctor found',
    doctorsFound: 'doctors found',
    finding: 'Finding doctors...',
    noResults: 'No doctors found',
    noResultsHint: 'Try adjusting your search or filters to find more results',
    clearFilters: 'Clear all filters',
    from: 'from',
    insurance: 'Insurance',
    noInsurance: 'No insurance',
  },

  // Doctor Filters
  filter: {
    refine: 'Refine Results',
    clearAll: 'Clear all',
    city: 'City',
    clinic: 'Clinic',
    specialty: 'Specialty',
    insuranceOnly: 'Only show doctors that accept insurance',
    availabilityLabel: 'Availability',
    availabilityAny: 'Any time',
    availabilityThisWeek: 'This week',
    availabilityDate: 'Pick a date',
  },

  // Doctor Profile Page
  doctorProfile: {
    back: 'Back to doctors',
    viewProfile: 'View profile',
    about: 'About',
    services: 'Services',
    reviews: 'Patient Reviews',
    book: 'Book Appointment',
    notFound: 'Doctor not found',
    notFoundHint: 'This doctor may no longer be active.',
  },

  // Booking Dialog
  booking: {
    title: 'Book an Appointment',
    loadingServices: 'Loading services...',
    loadingAvailability: 'Loading availability...',
    booked: 'Appointment Booked!',
    bookedSubtitle: 'You\'ll receive a confirmation shortly.',
    noServices: 'No services available',
    noServicesHint: 'This doctor hasn\'t listed any services yet.',
    noSlots: 'No available appointments',
    noSlotsHint: 'Please check back later.',
    noSlotsDay: 'No time slots available for this day.',
    stepSpecialty: 'Choose a specialty',
    step1: 'Choose a service',
    step2: 'Select a day',
    step3: 'Choose a time',
    confirm: 'Confirm Booking',
    clinicLocation: 'Clinic Location',
    min: 'min',
    authRequired: 'Please log in to book an appointment',
    slotUnavailable: 'This time slot is no longer available',
    alreadyBooked: 'You already have an appointment at this time',
    unknownError: 'Something went wrong. Please try again.',
  },

  // Profile page
  profile: {
    title: 'My Profile',
    appointments: 'Appointment history',
    appointmentsSubtitle: 'All appointments are kept on record for legal and medical purposes.',
    noAppointments: 'No appointments yet',
    noAppointmentsHint: 'Your booking history will appear here once you schedule a visit.',
    loadError: 'Could not load appointments. Please try again.',
    roleAdmin: 'Administrator',
    roleClinicAdmin: 'Clinic Admin',
    close: 'Close',
    cancel: 'Cancel',
    reBook: 'Book again',
    pendingReviews: 'Awaiting your review',
    editProfile: 'Edit profile',
    editProfileTitle: 'Edit profile',
    firstName: 'First name',
    lastName: 'Last name',
    username: 'Username',
    saveChanges: 'Save changes',
    saveSuccess: 'Profile updated successfully',
    uploadPhoto: 'Upload photo',
    uploadPhotoHint: 'JPG, PNG or WebP · Max 2 MB',
    uploadError: 'Could not upload image. Please try again.',
    updateError: 'Could not save changes. Please try again.',
    usernameConflict: 'This username is already taken.',
    status: {
      booked: 'Booked',
      cancelled: 'Cancelled',
    },
    detail: {
      title: 'Appointment details',
      doctor: 'Doctor',
      clinic: 'Clinic',
      service: 'Service',
      date: 'Date',
      time: 'Time',
      price: 'Price',
      bookedOn: 'Booked on',
    },
  },

  // Health tips (cycling card on auth page)
  healthTips: [
    'Aim for at least 10 minutes of walking after each meal.',
    'Stay hydrated – small sips throughout the day help more.',
    'Set reminders so you never miss a dose.',
    'Track how you feel every day — patterns help doctors.',
    'Take short stretch breaks every hour.',
    'Eat slowly — it helps control portions.',
    'Limit screens before bed for better sleep.',
    'A short nap can restore focus without grogginess.',
  ],

  // Auth pages
  auth: {
    login: {
      title: 'Welcome back',
      subtitle: 'Sign in to manage your appointments.',
      emailLabel: 'Email address',
      passwordLabel: 'Password',
      rememberMe: 'Remember me',
      signIn: 'Sign in',
      invalidCredentials: 'Invalid email or password',
      accountDeactivated: 'Your account has been deactivated. Please contact support.',
      successToast: 'Welcome back!',
    },
    register: {
      title: 'Create Account',
      subtitle: 'Fill in your details to get started.',
      firstName: 'First Name',
      lastName: 'Last Name',
      email: 'Email',
      username: 'Username',
      password: 'Password',
      confirmPassword: 'Confirm Password',
      submit: 'Create Account',
      error: 'Unable to create account. Please try again.',
      successToast: 'Account created! You can now sign in.',
    },
    panel: {
      newTitle: 'New Here?',
      newText: 'Create an account and start booking trusted doctors in minutes.',
      signUp: 'Sign up',
      welcomeTitle: 'Welcome Back',
      welcomeText: 'Already have an account? Sign in to continue where you left off.',
      signIn: 'Sign in',
    },
    validation: {
      required: '{field} is required',
      emailInvalid: 'Email must be valid',
      passwordMinLength: 'Password must be at least {min} characters',
      passwordUppercase: 'Password must contain at least one uppercase letter',
      passwordDigit: 'Password must contain at least one digit',
      passwordsMatch: 'Passwords must match',
      fieldEmail: 'Email',
      fieldPassword: 'Password',
      fieldFirstName: 'First name',
      fieldLastName: 'Last name',
      fieldUsername: 'Username',
      fieldConfirmPassword: 'Confirm password',
    },
  },

  // Locale switcher
  locale: {
    en: 'English',
    ro: 'Romanian',
  },

  // Specialties (keyed by slug)
  specialty: {
    'cardiology': 'Cardiology',
    'dermatology': 'Dermatology',
    'neurology': 'Neurology',
    'orthopedics': 'Orthopedics',
    'pediatrics': 'Pediatrics',
    'psychiatry': 'Psychiatry',
    'ophthalmology': 'Ophthalmology',
    'ent': 'ENT',
    'general-medicine': 'General Medicine',
    'endocrinology': 'Endocrinology',
    'gastroenterology': 'Gastroenterology',
    'urology': 'Urology',
    'gynecology': 'Gynecology',
    'pulmonology': 'Pulmonology',
    'rheumatology': 'Rheumatology',
    'oncology': 'Oncology',
    'nephrology': 'Nephrology',
    'infectious-diseases': 'Infectious Diseases',
    'allergy-and-immunology': 'Allergy and Immunology',
    'diabetology': 'Diabetology',
    'sports-medicine': 'Sports Medicine',
    'geriatrics': 'Geriatrics',
    'dentistry': 'Dentistry',
  },

  // Contact page
  contact: {
    chip: 'Get in touch',
    title: 'Contact us',
    subtitle: 'Have a question or need help? Send us a message and we\'ll get back to you within 24 hours.',
    email: 'Email',
    emailHint: 'Response within 24 hours',
    phone: 'Phone',
    address: 'Address',
    formTitle: 'Send us a message',
    name: 'Full name',
    yourEmail: 'Your email',
    subject: 'Subject',
    message: 'Message',
    messagePlaceholder: 'Tell us how we can help...',
    sendingAs: 'Sending as',
    send: 'Send message',
    successTitle: 'Message sent!',
    successText: 'Thank you for reaching out. We\'ll get back to you within 24 hours.',
    errorText: 'Something went wrong. Please try again.',
  },

  // Reviews
  review: {
    leave: 'Leave a review',
    edit: 'Edit review',
    title: 'Leave a Review',
    editTitle: 'Edit Your Review',
    rating: 'Your rating',
    comment: 'Comment (optional)',
    commentPlaceholder: 'Share your experience...',
    submit: 'Submit review',
    success: 'Review submitted!',
    noReviews: 'No reviews yet',
    reviews: 'reviews',
    basedOn: 'Based on',
    yourRating: 'Your rating',
    errorSubmit: 'Something went wrong. Please try again.',
    errorNotPast: 'You can only leave a review after the appointment has taken place.',
    delete: 'Delete review',
    deleteConfirm: 'Are you sure?',
    deleteConfirmBtn: 'Yes, delete',
    deleteSuccess: 'Review deleted',
    errorDelete: 'Could not delete the review. Please try again.',
  },

  // Admin panel
  admin: {
    nav: {
      panel: 'Admin Panel',
      doctors: 'Doctors',
      clinics: 'Clinics',
      appointments: 'Appointments',
      users: 'Users',
      content: 'Content & Data',
      analytics: 'Analytics',
      backToSite: 'Back to site',
    },
    analytics: {
      title: 'Analytics',
      subtitle: 'Overview of appointments, doctors, clinics and reviews',
      noData: 'No data available yet.',
      kpi: {
        total: 'Total Appointments',
        booked: 'Confirmed',
        cancellationRate: 'Cancellation Rate',
        cancelled: 'cancelled',
        avgRating: 'Avg. Rating',
        reviews: 'reviews',
      },
      period: {
        day: 'Last 30 days',
        week: 'Last 12 weeks',
        month: 'Last 12 months',
      },
      chart: {
        title: 'Appointments Over Time',
        booked: 'Booked',
        cancelled: 'Cancelled',
        noData: 'No appointments in the selected period.',
      },
      topDoctors: {
        title: 'Most Booked Doctors',
      },
      topSpecialties: {
        title: 'Most Booked Specialties',
      },
      clinicOccupancy: {
        title: 'Clinic Occupancy',
        hint: 'Based on time slots from the last 30 days',
        slots: 'slots booked',
      },
      reviewScores: {
        title: 'Review Scores',
        distribution: 'Rating Distribution',
        topRated: 'Top Rated Doctors',
      },
      revenue: {
        title: 'Revenue',
        totalRevenue: 'Total Revenue',
        avgRevenue: 'Avg. Revenue / Appointment',
        chart: 'Revenue Over Time',
        topByDoctor: 'Top Doctors by Revenue',
        topByService: 'Top Services by Revenue',
        ron: 'RON',
        noData: 'No revenue data yet.',
      },
    },
    appointments: {
      title: 'Appointments',
      total: 'total',
      patientSearch: 'Patient name or email',
      dateFrom: 'From',
      dateTo: 'To',
      allDoctors: 'All Doctors',
      allClinics: 'All Clinics',
      allStatuses: 'All Statuses',
      clearFilters: 'Clear',
      patient: 'Patient',
      doctor: 'Doctor',
      service: 'Service',
      dateTime: 'Date & Time',
      status: 'Status',
      actions: 'Actions',
      noAppointments: 'No appointments found.',
      statusBooked: 'Booked',
      statusCancelled: 'Cancelled',
      cancelTitle: 'Cancel Appointment',
      cancelText: 'Are you sure you want to cancel this appointment? The time slot will become available again.',
      cancelConfirm: 'Yes, Cancel',
      errorCancelFailed: 'Failed to cancel the appointment. Please try again.',
      detail: {
        title: 'Appointment Details',
        patient: 'Patient',
        doctor: 'Doctor',
        clinic: 'Clinic',
        service: 'Service',
        duration: 'Duration',
        price: 'Price',
        date: 'Date',
        time: 'Time',
        bookedOn: 'Booked on',
        review: 'Patient Review',
        noReview: 'No review submitted.',
        min: 'min',
      },
      create: {
        title: 'New Appointment',
        searchPatient: 'Search patient by name or email...',
        selectPatient: 'Select Patient',
        noPatients: 'No patients found.',
        searchDoctor: 'Search by name or clinic...',
        noDoctors: 'No doctors found.',
        selectDoctor: 'Select Doctor',
        selectService: 'Select Service',
        selectDate: 'Select Date',
        selectTime: 'Select Time',
        noSlots: 'No available slots for this period.',
        noSlotsDay: 'No slots for this day.',
        confirm: 'Confirm Booking',
        success: 'Appointment created successfully.',
        errorSlotTaken: 'This slot is no longer available.',
        errorCreate: 'Failed to create appointment. Please try again.',
      },
    },
    doctors: {
      title: 'Manage Doctors',
      addDoctor: 'Add Doctor',
      searchPlaceholder: 'Search by name or clinic...',
      name: 'Name',
      clinic: 'Clinic',
      specialties: 'Specialties',
      insurance: 'Insurance',
      active: 'Active',
      inactive: 'Inactive',
      actions: 'Actions',
      yes: 'Yes',
      no: 'No',
      total: 'doctors',
      availability: 'Availability',
    },
    clinics: {
      title: 'Manage Clinics',
      addClinic: 'Add Clinic',
      name: 'Name',
      city: 'City',
      address: 'Address',
      description: 'Description',
      actions: 'Actions',
      total: 'clinics',
    },
    doctorForm: {
      createTitle: 'Add Doctor',
      editTitle: 'Edit Doctor',
      tabInfo: 'Info',
      tabServices: 'Services',
      firstName: 'First Name',
      lastName: 'Last Name',
      bio: 'Bio',
      clinic: 'Clinic',
      specialties: 'Specialties',
      acceptsInsurance: 'Accepts Insurance',
      isActive: 'Active',
      workingHours: 'Working Hours',
      workDays: 'Work days',
      startHour: 'Start time',
      endHour: 'End time',
      mon: 'Mon', tue: 'Tue', wed: 'Wed', thu: 'Thu',
      fri: 'Fri', sat: 'Sat', sun: 'Sun',
    },
    clinicForm: {
      createTitle: 'Add Clinic',
      editTitle: 'Edit Clinic',
      name: 'Name',
      description: 'Description',
      address: 'Address',
      city: 'City',
    },
    services: {
      title: 'Assigned Services',
      pendingTitle: 'Services to be added',
      pendingHint: 'These services will be saved when the doctor is created.',
      noServicesPending: 'No services added yet.',
      filteredBySpecialty: 'Showing services for selected specialties',
      addService: 'Add Service',
      service: 'Service',
      price: 'Price (RON)',
      duration: 'Duration (min)',
      noServices: 'No services assigned.',
      save: 'Save',
      delete: 'Delete',
      confirmDelete: 'Delete this service?',
      errorConflict: 'Cannot delete: there are future active appointments for this service.',
      errorDuplicate: 'This service is already assigned to the doctor.',
    },
    form: {
      save: 'Save',
      cancel: 'Cancel',
      confirm: 'Confirm',
      create: 'Create',
      edit: 'Edit',
      delete: 'Delete',
      saving: 'Saving...',
      leaveTitle: 'Leave without saving?',
      leaveText: 'You have unsaved changes that will be lost.',
      leaveConfirm: 'Leave',
      stayHere: 'Stay',
      unsavedChanges: 'Unsaved changes',
    },
    content: {
      tabServices: 'Medical Services',
      tabSpecialties: 'Specialties',
      tabTools: 'Tools',
      services: {
        title: 'Medical Services',
        total: 'services',
        addService: 'Add Service',
        name: 'Name',
        slug: 'Slug',
        specialty: 'Specialty',
        noSpecialty: 'None',
        actions: 'Actions',
        createTitle: 'Add Medical Service',
        deleteTitle: 'Delete Medical Service',
        deleteConfirm: 'Are you sure you want to delete "{name}"? This cannot be undone.',
        slugHint: 'Auto-generated from name. Must be lowercase with hyphens only.',
      },
      specialties: {
        title: 'Specialties',
        total: 'specialties',
        addSpecialty: 'Add Specialty',
        name: 'Name',
        slug: 'Slug',
        actions: 'Actions',
        createTitle: 'Add Specialty',
        deleteTitle: 'Delete Specialty',
        deleteConfirm: 'Are you sure you want to delete "{name}"? This cannot be undone.',
        slugHint: 'Auto-generated from name. Must be lowercase with hyphens only.',
      },
      tools: {
        title: 'Tools',
        slotsTitle: 'Regenerate Time Slots',
        slotsText: 'Generates time slots for the next 30 days for all active doctors. Past unbooked slots are removed; existing future slots are skipped.',
        slotsBtn: 'Regenerate',
        slotsRunning: 'Regenerating...',
        slotsSuccess: 'Time slots regenerated successfully.',
        slotsError: 'Failed to regenerate time slots. Please try again.',
      },
    },
    users: {
      title: 'Manage Users',
      searchPlaceholder: 'Search by name or email...',
      name: 'Name',
      email: 'Email',
      username: 'Username',
      role: 'Role',
      clinic: 'Clinic',
      status: 'Status',
      actions: 'Actions',
      total: 'users',
      active: 'Active',
      inactive: 'Inactive',
      admin: 'Admin',
      roleClinicAdmin: 'Clinic Admin',
      roleDoctor: 'Doctor',
      user: 'User',
      deactivate: 'Deactivate',
      activate: 'Activate',
      deactivateTitle: 'Deactivate Account',
      activateTitle: 'Activate Account',
      deactivateText: 'This user will no longer be able to log in. Their data and appointments are preserved.',
      activateText: 'This user will be able to log in again.',
      deactivateConfirm: 'Yes, Deactivate',
      activateConfirm: 'Yes, Activate',
      errorAction: 'Action failed. Please try again.',
      makeClinicAdmin: 'Make Clinic Admin',
      removeClinicAdmin: 'Remove Clinic Admin',
      makeClinicAdminTitle: 'Assign Clinic Admin Role',
      makeClinicAdminClinic: 'Select Clinic',
      makeClinicAdminConfirm: 'Assign',
      assignDoctor: 'Link Doctor Profile',
      assignDoctorTitle: 'Link to Doctor Profile',
      assignDoctorSelect: 'Select Doctor',
      assignDoctorConfirm: 'Link',
      removeDoctor: 'Remove Doctor Link',
      removeDoctorTitle: 'Remove Doctor Link',
    },
    errors: {
      loadFailed: 'Failed to load data. Please try again.',
      saveFailed: 'Failed to save. Please try again.',
      deleteFailed: 'Failed to delete. Please try again.',
      uploadFailed: 'Failed to upload image. Please try again.',
    },
    availability: {
      title: 'Doctor Availability',
      workSchedule: 'Work Schedule',
      workDays: 'Working days',
      hours: 'Hours',
      noPeriods: 'No unavailability periods set.',
      blockDates: 'Block Dates',
      dateFrom: 'From',
      dateTo: 'To',
      reason: 'Reason (optional)',
      add: 'Block',
      deleteConfirm: 'Remove this unavailability period? Slots will be regenerated for the affected dates.',
      overlapError: 'This date range overlaps with an existing unavailability period.',
      saveError: 'Failed to save unavailability. Please try again.',
      deleteError: 'Failed to remove period. Please try again.',
      mon: 'Mon', tue: 'Tue', wed: 'Wed', thu: 'Thu',
      fri: 'Fri', sat: 'Sat', sun: 'Sun',
    },
  },

  // Doctor dashboard
  doctor: {
    nav: {
      panel: 'Doctor Panel',
      dashboard: 'Schedule',
      backToSite: 'Back to site',
      logout: 'Log out',
    },
    schedule: 'Schedule',
    today: 'Today',
    todayLabel: 'Today\'s appointments',
    viewingDate: 'Viewing appointments for {date}',
    refresh: 'Refresh',
    startConsultation: 'Start',
    endConsultation: 'End',
    pauseConsultation: 'Undo start (revert to booked)',
    reopenConsultation: 'Reopen (revert to in progress)',
    noAppointments: 'No appointments',
    noAppointmentsHint: 'There are no appointments scheduled for this date.',
    loadError: 'Could not load appointments. Please try again.',
    errorAction: 'Action failed. Please try again.',
    min: 'min',
    delayed: '+{minutes} min delay',
    effectiveTime: 'Expected time',
    overTimeWarning: '⚠ You are running {minutes} minute(s) over schedule. Please wrap up.',
    overTimeMinutes: '{minutes} min over',
    consultationEnded: 'Consultation Ended',
    endedLate: 'This consultation ended {minutes} minute(s) late.',
    endedOnTime: 'Consultation ended on time.',
    notifiedPatients: '{count} patient(s) notified of the delay.',
    cancelTitle: 'Cancel Appointment',
    cancelText: 'Cancel the appointment for {patient} at {time}? The time slot will become available again.',
    cancelConfirm: 'Yes, Cancel',
    status: {
      booked: 'Booked',
      in_progress: 'In Progress',
      completed: 'Completed',
      cancelled: 'Cancelled',
    },
  },

  // Medical services (keyed by slug for translating API data)
  services: {
    'consultation': 'Consultation',
    'follow-up': 'Follow-up',
    'ecg': 'ECG',
    'echocardiography': 'Echocardiography',
    'holter-ecg-24h': 'Holter ECG 24h',
    'blood-pressure-monitoring-24h': 'Blood Pressure Monitoring 24h',
    'dermatoscopy': 'Dermatoscopy',
    'cryotherapy': 'Cryotherapy',
    'skin-biopsy': 'Skin Biopsy',
    'dental-scaling': 'Dental Scaling',
    'teeth-whitening': 'Teeth Whitening',
    'dental-filling': 'Dental Filling',
    'tooth-extraction': 'Tooth Extraction',
    'root-canal-treatment': 'Root Canal Treatment',
    'dental-crown': 'Dental Crown',
    'fundus-examination': 'Fundus Examination',
    'tonometry': 'Tonometry',
    'perimetry': 'Perimetry',
    'audiometry': 'Audiometry',
    'nasal-endoscopy': 'Nasal Endoscopy',
    'upper-endoscopy': 'Upper Endoscopy',
    'colonoscopy': 'Colonoscopy',
    'abdominal-ultrasound': 'Abdominal Ultrasound',
    'joint-injection': 'Joint Injection',
    'pelvic-ultrasound': 'Pelvic Ultrasound',
    'pap-smear': 'Pap Smear',
    'renal-ultrasound': 'Renal Ultrasound',
    'uroflowmetry': 'Uroflowmetry',
    'eeg': 'EEG',
    'electromyography': 'Electromyography',
    'thyroid-ultrasound': 'Thyroid Ultrasound',
    'spirometry': 'Spirometry',
    'pediatric-consultation': 'Pediatric Consultation',
    'vaccination': 'Vaccination',
  },
}
